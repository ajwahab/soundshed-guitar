...
fs::path WriteStereoWav(const std::vector<float>& left,
...
bool TestQualityChangeIsNotBlocking()
{
  ...
  std::cout << (ok ? "OK" : "FAILED")
            << " (worst=" << std::fixed << std::setprecision(1) << worstMs
            << " ms on " << worstLabel << ", budget=" << kRebuildBudgetMs
            << " ms; load=" << loadMs << " ms)\n";
  return ok;
}
...
bool TestReverbStillWetAfterQualityChanges()
{
  ...
  for (const double quality : { 0.0, 3.0, 1.0, 2.0 })
  {
    ...
    {
      std::cout << "FAILED (output collapsed at quality " << quality
                << ": rms " << before.rms << " -> " << after.rms << ")\n";
      return false;
    }
  }
...
bool TestRedundantQualityWriteIsIgnored()
{
  ...
  std::cout << (ok ? "OK" : "FAILED")
            << " (redundant=" << std::fixed << std::setprecision(3) << redundantMs
...
bool TestOversizedBlockFillsWholeBuffer()
{
  ...
  if (untouched > 0)
  {
    std::cout << "FAILED (" << untouched << "/" << oversized
              << " output samples left stale)\n";
    return false;
  }
...
int main()
{
  ...
  for (auto* test : { &TestQualityChangeIsNotBlocking,
                      &TestReverbStillWetAfterQualityChanges,
                      &TestRedundantQualityWriteIsIgnored,
                      &TestOversizedBlockFillsWholeBuffer })
  {
    if ((*test)())
      ++passed;
    else
