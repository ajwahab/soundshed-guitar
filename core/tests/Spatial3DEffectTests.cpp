...
Stereo Sine(std::size_t samples, double frequency, double amplitude = 0.5)
{
  ...
  {
    const auto v = static_cast<float>(amplitude * std::sin(2.0 * kPi * frequency * static_cast<double>(i) / kSampleRate));
    s.left[i] = v;
    s.right[i] = v;
  }
  return s;
...
double CrossCorrelationLag(const std::vector<float> &left, const std::vector<float> &right,
                           int maxLag, std::size_t skip)
{
...
void TestInterauralTimeDifference()
{
  ...
  Check(std::fabs(lagCentre) <= 1.0, "centred source has no interaural delay",
        Num(lagCentre, 1) + " samples");
  Check(lagRight > 0.0, "source on the right reaches the right ear first",
        Num(lagRight, 1) + " samples");
  Check(lagLeft < 0.0, "source on the left reaches the left ear first",
        Num(lagLeft, 1) + " samples");
  ...
  Check(std::fabs(lagOverhead) <= 1.0, "overhead source has no lateral delay",
        Num(lagOverhead, 1) + " samples");
  ...
  Check(std::fabs(lagSpeakers) <= 1.0, "speaker mode removes interaural delay",
        Num(lagSpeakers, 1) + " samples");
}
...
void TestDistance()
{
  ...
  Check(Db(nearHigh / farHigh) > Db(nearLow / farLow) + 2.0,
        "distance attenuates high frequencies faster than low",
...
void TestLatencyAndDryBlend()
{
  ...
  {
    worst = std::max(worst, static_cast<double>(std::fabs(out.left[i] - input.left[i - static_cast<std::size_t>(latency)])));
    worst = std::max(worst, static_cast<double>(std::fabs(out.right[i] - input.right[i - static_cast<std::size_t>(latency)])));
  }
...
void TestMotionOffMatchesStatic()
{
  ...
  Check(worst < 1.0e-6, "zero-depth motion is identical to a static position",
        "max deviation " + Num(worst, 9));
}
...
void TestMotionRateAndSync()
{
  ...
  Check(std::fabs(effect->GetParam("effectiveRate") - 0.5) < 1.0e-6,
        "one bar at 120 BPM is a two second revolution", Num(effect->GetParam("effectiveRate"), 4) + " Hz");
  effect->SetParam("bpm", 240.0);
  Check(std::fabs(effect->GetParam("effectiveRate") - 1.0) < 1.0e-6,
        "doubling the tempo doubles the motion rate", Num(effect->GetParam("effectiveRate"), 4) + " Hz");
}
...
void TestPitchStabilityDuringMotion()
{
  ...
  Check(dopplerApproach > smoothApproach * 5.0,
        "doppler mode bends pitch as the source approaches and recedes",
...
void TestEdgeCases()
{
  ...
  Check(PeakAbs(tail) == 0.0f, "Reset clears every delay line and filter",
        "residual peak " + Num(PeakAbs(tail), 9));
}
...
void TestStereoPreservationContract()
{
  ...
  Check(effect->ProducesStereoOutput(),
        "declares stereo output so the graph will not collapse the image downstream");
