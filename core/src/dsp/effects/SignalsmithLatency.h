...
namespace guitarfx
{
   ...
   */
  [[nodiscard]] inline int SignalsmithTotalLatencySamples(
    const signalsmith::stretch::SignalsmithStretch<float>& stretch)
  {
    return static_cast<int>(stretch.inputLatency() + stretch.outputLatency());
  }

  [[nodiscard]] inline int SignalsmithTotalLatencySamples(
    const signalsmith::stretch::SignalsmithStretch<float>& a,
