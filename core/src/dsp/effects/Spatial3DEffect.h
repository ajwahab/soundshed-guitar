...
namespace guitarfx
{
  ...
  public:
    void Prepare(double sampleRate, int maxBlockSize) override
    {
      ...
      mMaxShadowGroupDelay = static_cast<double>(maxShadowCoeff) / std::max(1.0e-6, 1.0 - static_cast<double>(maxShadowCoeff));
    ...
    void SetParam(const std::string &key, double value) override
    {
      if (key == "azimuth")
        ...
        mMotionMode.store(static_cast<int>(std::clamp(value, 0.0, static_cast<double>(spatial3d::kMotionModeCount - 1))),
                          std::memory_order_relaxed);
    ...
    [[nodiscard]] bool SupportsMonoProcessing() const override { return false; }
    [[nodiscard]] int GetLatencySamples() const override { return mPrepared ? mBiasSamples : 0; }

    [[nodiscard]] std::string GetType() const override { return "spatial_3d"; }
  ...
  private:
    ...
    [[nodiscard]] double EffectiveMotionRateHz() const
    {
      ...
      return std::clamp(tempo_sync::DivisionRateHz(bpm, division),
                        spatial3d::kMinMotionRateHz, spatial3d::kMaxMotionRateHz);
    }

    /** Deterministic hash used to give Drift mode a reproducible trajectory. */
    static double SeedPhase(int seed, int index)
    {
      const double x = std::sin(static_cast<double>(seed) * 12.9898 + static_cast<double>(index) * 78.233) * 43758.5453;
    ...
    void AdvanceMotionAndSmoothing(int len)
    {
      ...
      if (mode != spatial3d::kMotionOff)
      {
        ...
        const auto angle = [this, dir](std::size_t index)
        { return spatial3d::kTwoPi * mPhases[index] * dir; };
        ...
        case spatial3d::kMotionDrift:
          azOffset = depth * 120.0 * (0.6 * std::sin(angle(3) + mDriftPhase[0]) +
                                      0.4 * std::sin(angle(4) + mDriftPhase[1]));
      ...
      const double targetDist = std::clamp(anchorDist * distMultiplier,
                                           spatial3d::kMinDistanceM, spatial3d::kMaxDistanceM);
    ...
    [[nodiscard]] Coefficients ComputeCoefficients(bool speakers, float roomAmount)
    {
      ...
      const double notchFreq = std::exp(std::log(6000.0) + (std::log(11000.0) - std::log(6000.0)) * (elNorm + 1.0) * 0.5);
    ...
    static double ShadowCutoff(double shadowAmount)
    {
      ...
      return spatial3d::kMaxShadowCutoffHz
             * std::pow(spatial3d::kMinShadowCutoffHz / spatial3d::kMaxShadowCutoffHz,
  ...
  namespace spatial3d
  {
     ...
     */
    inline EffectPresetDefinition MakePreset(
        const std::string &id, const std::string &displayName,
        double azimuth, double elevation, double distance,
        int motionMode, double motionRate, double motionDepth,
        double motionElevDepth, double motionDistDepth, double motionSmooth,
        double roomAmount, int listenMode, double motionPhase = 0.0,
        int motionDirection = 0, int syncMode = tempo_sync::kSyncModeOff,
        int syncDivision = 0, int motionSeed = 1)
    {
      ...
      preset.parameterOrder = {
          ...
          "motionMode", "motionRate", "syncMode", "syncDivision",
          "motionDepth", "motionElevDepth", "motionDistDepth",
          "motionDirection", "motionPhase", "motionSmooth", "motionSeed",
          "roomAmount", "listenMode", "delayMode", "mix", "outputTrim"};
      return preset;
    }
    ...
    {
      return {
          //          id                  display                az     el   dist  mode              rate    depth  elev   dist   smooth room  listen
          MakePreset("static-centre", "Static · Centre", 0.0, 0.0, 1.5, kMotionOff, 0.06, 0.0, 0.0, 0.0, 0.40, 0.20, kListenHeadphones),
          MakePreset("gentle-orbit", "Gentle Orbit", 0.0, 0.0, 1.8, kMotionOrbit, 0.0625, 0.60, 0.25, 0.30, 0.55, 0.30, kListenHeadphones),
          MakePreset("slow-carousel", "Slow Carousel", 0.0, 0.0, 2.2, kMotionOrbit, 0.0333, 1.00, 0.10, 0.20, 0.70, 0.35, kListenHeadphones),
          MakePreset("wide-sway", "Wide Sway", 0.0, 0.0, 1.6, kMotionFigureEight, 0.14, 0.85, 0.00, 0.00, 0.45, 0.20, kListenHeadphones),
          MakePreset("overhead-arc", "Overhead Arc", 0.0, 0.0, 1.8, kMotionArc, 0.0625, 0.50, 1.00, 0.25, 0.50, 0.30, kListenHeadphones),
          MakePreset("figure-of-eight", "Figure of Eight", 0.0, 0.0, 1.8, kMotionFigureEight, 0.0833, 0.80, 0.70, 0.00, 0.45, 0.25, kListenHeadphones),
          MakePreset("fly-by", "Fly-By", 0.0, 0.0, 2.5, kMotionPendulum, 0.04, 1.00, 0.30, 0.70, 0.60, 0.40, kListenHeadphones),
          MakePreset("ambient-drift", "Ambient Drift", 0.0, 0.0, 2.4, kMotionDrift, 0.025, 0.70, 0.50, 0.40, 0.85, 0.45, kListenHeadphones),
          MakePreset("rising-spiral", "Rising Spiral", 0.0, -20.0, 2.0, kMotionSpiral, 0.05, 0.60, 0.80, 0.35, 0.60, 0.35, kListenHeadphones),
          MakePreset("tempo-orbit-bar", "Tempo Orbit · 1 Bar", 0.0, 0.0, 1.8, kMotionOrbit, 0.5, 0.60, 0.20, 0.20, 0.50, 0.30, kListenHeadphones,
                     0.0, 0, tempo_sync::kSyncModeTempo, 0),
          MakePreset("speaker-safe-sway", "Speaker Safe Sway", 0.0, 0.0, 1.5, kMotionFigureEight, 0.10, 0.75, 0.00, 0.10, 0.50, 0.15, kListenSpeakers)};
    }
  } // namespace spatial3d
  ...
  inline void RegisterSpatial3DEffect()
  {
    ...
    info.parameters = {
        ...
        {"distance", "Distance", spatial3d::kReferenceDistanceM, spatial3d::kMinDistanceM,
         spatial3d::kMaxDistanceM, "m", "Position", false, 0.05, {}},

        {"motionMode", "Motion", 0.0, 0.0, static_cast<double>(spatial3d::kMotionModeCount - 1), "enum",
         "Motion", false, 1.0, spatial3d::MotionModeLabels()},
        {"motionRate", "Rate", 0.06, spatial3d::kMinMotionRateHz, spatial3d::kMaxMotionRateHz, "Hz",
         "Motion", false, 0.01, {}},
    ...
    EffectRegistry::Instance().Register(info.type, info, []()
                                        { return std::make_unique<Spatial3DEffect>(); });
  }
