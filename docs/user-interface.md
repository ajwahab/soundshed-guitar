...
- **Models** — glTF 2.0 components in `core/ui/assets/models/` (`amp-head`, `amp-knob`,
  ...
  geometry plus named material slots only (`extras.materialSlot`); all textures are
  generated procedurally at runtime from canvas so the silkscreen labels follow the node's
  real parameter list and the repo stays small.
...
- **Cabinet** — a 4x12 is stacked under the head when `nodeUsesFullRigNamCategory(node)` is
  true (i.e. the loaded capture is a full rig). The viewport switches to a 5:4 aspect ratio
  in that case.
...
- **Fallbacks** — parameters that cannot be a physical knob (toggles, enums, advanced params
  and any beyond the panel's knob capacity) are still rendered as standard HTML controls
  under the viewport, so no control is ever lost. If WebGL is unavailable the toggle is not
  offered at all, and if the models fail to load the viewport shows an inline error telling
  the user to switch back.
- Rendering is on demand only (a `requestAnimationFrame` per state change), and the view
  instance is reused across parameter-panel re-renders rather than being rebuilt.
